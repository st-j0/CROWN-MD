   ...
   if (id.endsWith('g.us')) {
      for (let jid in presences) {
         ...
         if ((presences[jid].lastKnownPresence === 'composing' || presences[jid].lastKnownPresence === 'recording') && global.db && global.db.users && global.db.users.find(v => v.jid == jid) && global.db.users.find(v => v.jid == jid).afk > -1) {
            sock.reply(id, `Le système détecte une activité de @${jid.replace(/@.+/, '')} après avoir été hors ligne pendant : ${Func.texted('bold', Func.toTime(new Date - global.db.users.find(v => v.jid == jid).afk))}\n\n➠ ${Func.texted('bold', 'Reason')} : ${global.db.users.find(v => v.jid == jid).afkReason ? global.db.users.find(v => v.jid == jid).afkReason : '-'}`, global.db.users.find(v => v.jid == jid).afkObj)
...
client.on('group.add', async ctx => {
   const sock = client.sock
   const text = `Merci +tag d'avoir rejoint le groupe +grup.`
   ...
   if (groupSet && groupSet.localonly) {
      if (global.db.users.some(v => v.jid == ctx.member) && !global.db.users.find(v => v.jid == ctx.member).whitelist && !ctx.member.startsWith('1') || !ctx.member.startsWith('253')) {
         sock.reply(ctx.jid, Func.texted('bold', `Sorry @${ctx.member.split`@`[0]}, ce groupe est réservé aux continent africain et l'amerique vous serez automatiquement supprimé.`))
