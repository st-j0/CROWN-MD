...
module.exports = async (client, ctx) => {
   ...
   try {
      ...
      if (m.isGroup && !groupSet.stay && (new Date * 1) >= groupSet.expired && groupSet.expired != 0) {
         return client.reply(m.chat, Func.texted('italic', '🚩 Le temps du robot a expiré et va quitter ce groupe, merci.', null, {
      ...
      if (users && (new Date * 1) >= users.expired && users.expired != 0) {
         return client.reply(users.jid, Func.texted('italic', '🚩 Votre forfait premium a expiré, merci davoir acheté et utilisé notre service.')).then(async () => {
            users.premium = false
            users.expired = 0
      ...
      if (m.isGroup && !m.isBot && users && users.afk > -1) {
         client.reply(m.chat, `Vous êtes de nouveau en ligne après avoir été hors ligne pendant : ${Func.texted('bold', Func.toTime(new Date - users.afk))}\n\n• ${Func.texted('bold', 'Reason')}: ${users.afkReason ? users.afkReason : '-'}`, m)
         users.afk = -1
         users.afkReason = ''
      }
      cron.schedule('00 00 * * *', () => {
      ...
      if (body && !setting.self && core.prefix != setting.onlyprefix && commands.includes(core.command) && !setting.multiprefix && !env.evaluate_chars.includes(core.command)) return client.reply(m.chat, `🚩 *Préfixe incorrect!*, ce bot utilise un préfixe : *[ ${setting.onlyprefix} ]*\n\n➠ ${setting.onlyprefix + core.command} ${text || ''}`, m)
      ...
      if (body && prefix && commands.includes(command) || body && !prefix && commands.includes(command) && setting.noprefix || body && !prefix && commands.includes(command) && env.evaluate_chars.includes(command)) {
         if (setting.error.includes(command)) return client.reply(m.chat, Func.texted('bold', `🚩 Commande _${(prefix ? prefix : '') + command}_ désactivé.`), m)
         ...
         for (let name in is_commands) {
            ...
            if (!m.isGroup && !['owner', 'menfess', 'scan', 'verify', 'payment', 'premium'].includes(name) && chats && !isPrem && !users.banned && setting.groupmode) {
               client.sendMessageModify(m.chat, `⚠️ Utilisation du bot dans le chat privé uniquement pour les utilisateurs premium, souhaitez-vous passer au forfait premium ? envoyez *${prefixes[0]}premium* pour voir les avantages et les prix.`, m, {
                  largeThumb: true,
            ...
            if (cmd.restrict && !isPrem && !isOwner && text && new RegExp('\\b' + setting.toxic.join('\\b|\\b') + '\\b').test(text.toLowerCase())) {
               client.reply(m.chat, `⚠️ Vous avez violé les *Conditions générales* d'utilisation de robots en utilisant des mots-clés sur liste noire, en guise de pénalité pour le blocage et le bannissement de votre violation..`, m).then(() => {
                  users.banned = true
            ...
            if (cmd.limit && users.limit < 1) {
               client.reply(m.chat, `⚠️ Vous avez atteint la limite et sera réinitialisée à minuit.\n\nPour obtenir plus de limites, passez aux forfaits premium.`, m).then(() => users.premium = false)
               continue
            }
            if (cmd.limit && users.limit > 0) {
               ...
               if (users.limit >= limit) {
                  ...
                  client.reply(m.chat, Func.texted('bold', `⚠️ Votre limite n'est pas suffisante pour utiliser cette fonctionnalité.`), m)
                  continue
               }
            }
         ...
         for (let name in is_events) {
            ...
            if (!m.isGroup && env.blocks.some(no => m.sender.startsWith(no))) return client.updateBlockStatus(m.sender, 'block')
            if (setting.self && !['menfess_ev', 'anti_link', 'anti_tagall', 'anti_virtex', 'filte'].includes(event.pluginName) && !isOwner && !m.fromMe) continue
            ...
            if (event.limit && !event.game && users.limit < 1 && body && Func.generateLink(body) && Func.generateLink(body).some(v => Func.socmed(v))) return client.reply(m.chat, `⚠️ Vous avez atteint la limite et sera réinitialisée à 00h00.\n\nPour obtenir plus de limites, passez au forfait premium.`, m).then(() => {
               users.premium = false
               users.expired = 0
            })
      ...
      console.log(e)
      if (!m.fromMe) return m.reply(Func.jsonFormat(new Error('sukunabot encountered an error :' + e)))
   }
